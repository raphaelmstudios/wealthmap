...
const ICONS = {
  ...
  JOB: `<svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><rect x="2" y="7" width="20" height="14" rx="2"/><path d="M16 7V5a2 2 0 0 0-2-2h-4a2 2 0 0 0-2 2v2"/><line x1="12" y1="12" x2="12" y2="12"/><line x1="8" y1="12" x2="16" y2="12"/></svg>`,
...
async function buildWealthMap() {
  ...
  // Hide form, scroll to top
  document.getElementById("resultsSection").style.display = "none";
  window.scrollTo({ top: 0, behavior: "smooth" });

  try {
    // These functions we build next, one by one
...
// ── BLOCK 2: Get Exchange Rate ──
async function getExchangeRate(countryCode) {
  try {
    const currency = COUNTRY_DATA[countryCode]?.currency || "USD";
    const url = `https://v6.exchangerate-api.com/v6/${CONFIG.EXCHANGE_RATE_KEY}/latest/USD`;
    ...
    console.warn("Exchange rate fetch failed, using defaults:", err);
...
// ── BLOCK 3: Investment Data by Country ──
function getInvestmentCards(countryCode, fx) {
  const { symbol, currency } = fx;
  const amount = window.userProfile.amount;
...
// ── BLOCK 5: Alpha Vantage Live Prices ──
async function getLivePrices(fx) {
  try {
    // Fetch Bitcoin price in USD
    const btcUrl = `https://www.alphavantage.co/query?function=CURRENCY_EXCHANGE_RATE&from_currency=BTC&to_currency=USD&apikey=${CONFIG.ALPHA_VANTAGE_KEY}`;
    const btcRes = await fetch(btcUrl);
    const btcData = await btcRes.json();

    const btcUSD = parseFloat(
      btcData["Realtime Currency Exchange Rate"]?.["5. Exchange Rate"] || 0,
    );
    ...
    return {
      btcUSD: 0,
      btcLocal: 0,
      btcFormatted: "Price unavailable",
    };
  }
}

// ── BLOCK 6: Claude AI Insight ──
...
Write in flowing paragraphs only. No bold text, no headers, no bullet points, no numbering, no markdown formatting of any kind. Just natural, conversational sentences that flow into each other like a trusted advisor speaking directly to them.`;
    ...
    console.warn("Claude API failed:", err);
...
async function generateResults() {
  ...
  // Inject live BTC price into crypto card
...
// ── BLOCK 7: Search + Filter Combined ──
...
function validateAmount(amount, currency) {
  if (amount > 1000000000) {
    return "Please enter a realistic investment amount.";
  }
  if (amount === 0) {
    return "Amount is zero — we will show you opportunities to earn first.";
  }
  return null;
}

// Show API status messages during loading
